...
# This script trains a linear model at each grid cell with monthly resolutions 
...
### remove 1990-2019 monthly mean linear trend from a datset : (the data might extend beyond 2020)
...
def poli(data):
  ...
  extend = xr.DataArray(np.arange(1990,1990+len(data['time'])/12,1/12), dims = 'time')
...
## you can change this function to any preprocessing function on the predictor data or any non linear function such as log() or sigmoid():
def normalize(ds):
    
    return ds


### Define the predictors with CHL

variables =  ['atm_anom'  ,'chl_anom' , 'sss_anom' ,'sst_anom','atm_det', 'wind_anom']

### define linear function with CHL on each grid cell. Here obs is the predictant.
...
### define linear function without CHL on each grid cell. Here obs is the predictant.
...
### define regress function to use xarray's apply_ufunc for wrapping the above regression functions over each grid point. This function outputs coefficients 
### of regression at each grid cell. Here obs is the predictant.
...
###  use the regression coefficients above to make predictions using the linear model based on any predictor values.
...
obs = xr.open_dataset('/fs/ccchome/acrnrpg/DCPP-local/Linear_regression/obs_predictors.nc').sel(time = slice(f'{1990}',f'{2019}')).drop('mld')

### deseasonalized predictors 
obs['atm_anom'] = deseason(obs.atm)
obs['sst_anom'] = deseason(obs.sst)
obs['sss_anom'] = deseason(obs.sss)
obs['chl_anom'] = deseason(obs.chl)

### Loading ERA5 wind product
wind = xr.open_dataset('/fs/ccchome/acrnrpg/DCPP-local/wind/wind_obs_1982-2020.nc').wind.sel(time = slice(f'{1990}',f'{2019}'))

### Adding observed atmospheric concentrations of CO2 detrended:
obs = xr.combine_by_coords([obs, (obs['atm_anom'] - poli(obs['atm_anom'])[1]).to_dataset(name = 'atm_det')])
### Adding observed atmospheric concentrations of CO2 linear trend:
obs['atm_anom'] = poli(obs['atm_anom'])[1]
### Adding deseasonalized wind:
obs = xr.combine_by_coords([obs, wind, deseason(wind).to_dataset(name = 'wind_anom')])

###################### CanESM5 bias corrected historical predictors #################
### Exactly like above, but this time we load bias corrected historical predictors from CanESM5. Code for bias correction is available in a different script.

hist = xr.open_dataset(f'/fs/ccchome/acrnrpg/DCPP-local/Final/CanESM5_historical_predictors_EM_1980_2020_bias_corrected.nc').sel(time = slice('1990','2019')).drop(['mld'])
hist['sst_anom'] = deseason(hist.sst)
hist['sss_anom'] = deseason(hist.sss)
hist['chl_anom'] = deseason(hist.chl)
wind = xr.open_dataset(f'/fs/ccchome/acrnrpg/DCPP-local/wind/CanESM5_historical_wind_bias_corrected.nc').wind.sel(time = slice(f'{1990}',f'{2019}'))
...
### Exactly like above, but this time we load bias corrected hindcast predictors from CanESM5 on lead years 1 to 10. The target period is fixed to 1990-2019.


ds_dict = {}
apco2 =  xr.open_dataset(f'/fs/ccchome/acrnrpg/DCPP-local/Temporary/CMIP6_atm_forecast_1982-2029.nc').ssp245.sel(time = slice('1990',None))
for i in range(1,11):
    

    ds_dict[i] = xr.open_dataset(f'/fs/ccchome/acrnrpg/DCPP-local/Final/CanESM5_hindcast_predictors_ly{i}_EM_1980_{2019+i}_bias_corrected.nc').sel(time = slice('1990',None)).drop(['mld'])
...
assim = xr.open_dataset('/fs/ccchome/acrnrpg/DCPP-local/assim_data/CanESM5_assim_predictors_r1i1p2f1_1980-2020_bias_corrected.nc').sel(time = slice('1990','2019')).drop(['mld'])
assim['sst_anom'] = deseason(assim.sst)
assim['sss_anom'] = deseason(assim.sss)
assim['chl_anom'] = deseason(assim.chl)
wind = xr.open_dataset(f'/fs/ccchome/acrnrpg/DCPP-local/assim_data/CanESM5_assim_sfcWind_1980-2020_r1i1p2f1.nc').wind.sel(time = slice('1990','2019'))
...
### Iterate over each of the six SeaFlux data products and train the a linear model each time.
for prd in sink.product:
    ...
    predictors = obs.where(~np.isnan(target)) ### removing target nan point from observational predictors
    ...
    recons.to_dataset(name = 'fgco2').to_netcdf(f'Linear_models/SeaFlux/Mean/recons_MEAN_linear.nc') # Save reconstruction using prd as predictant
    ...
    historical.to_dataset(name = 'fgco2').to_netcdf(f'Linear_models/SeaFlux/Mean/hist_MEAN_linear.nc') # Save historical using prd as predictant
    ...
    assimilation.to_dataset(name = 'fgco2').to_netcdf(f'Linear_models/SeaFlux/Mean/assim_MEAN_annual_lienar.nc') # Save assimilation using prd as predictant
    ...
    for i in range(1,11):
        ...
        hindcast_dict[i].to_dataset(name = 'fgco2').to_netcdf(f"Multimodel_Ensemble/Mean/hindcast_MEAN_linear_ly{i}.nc")



#### Train for the mean of SeaFlux
...
for i in range(1,11):
    ...
    # hindcast_dict_MPI[i] = ds.combine_first(ds2) 
    hindcast_dict[i] = ds2   #### MIROC 
    hindcast_dict[i].to_dataset(name = 'fgco2').to_netcdf(f"Multimodel_Ensemble/Mean/hindcast_MEAN_linear_ly{i}.nc")