...
### define a smoother function like in Landschutzer et al. (2016) using a 3x3 lat-lon roling mean window. 
...
def biome_predictor(predictors, b, CHL):
    ...
    if CHL:
        ...
        obs_biome = predictors.where(biomes == b).stack(x = ['time','lat','lon']).dropna(dim = 'x')
        out = np.c_[ obs_biome.atm_anom.values,  obs_biome.chl_anom.values, obs_biome.sss_anom.values , obs_biome.sst_anom.values, obs_biome.wind_anom.values]
...
### make predictions at a given biome b using predictors and the saved models. 
...
### make predictions at a given leadyear ly using predictors and the saved models. 
...
for prd in sink.product:
    ...
    for i in range(1,17):
      ...
      ### Creat an output with the same coverage as the predictant
    recons = ds.combine_first(xr.full_like(sink.sel(product = prd).resample(time = 'Y').mean(), fill_value = np.nan))  
    recons[:,:,:] = smoother(recons) ### Smooth the output data in lat x lon space
    recons.to_dataset(name = 'fgco2').to_netcdf(f'NN_models/SeaFlux/{prd}/Output/recons_{prd}_annual_SOMFFN_sigmoid15.nc') ### Save

    ######### Use the models  to make predictions using CanESM5 historical predictors 
    ...
    ######### Use the models  to make predictions using CanESM5 hindcast predictors 
    ...
    ######### Use the models to make predictions using CanESM5 assimilation predictors 
    for i in range(1,17):
...
#### Use the Mean of SeaFlux products
